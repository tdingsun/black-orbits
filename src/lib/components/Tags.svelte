...
<div class="flex sm:flex-col gap-4 sm:gap-1 select-none">
    ...
    <div class="flex  flex-wrap gap-4 sm:gap-0.5 sm:gap-x-2">
        ...
        <div onclick={() => toggleTag(tag)}
            class="text-sm  cursor-pointer px-1  {tags.selectedTag === tag ? '' : 'sm:hover:underline'} sm:px-0  relative border w-max  py-0.5 rounded-xs whitespace-nowrap  {tags.selectedTag === tag ? ' border-primary-text sm:px-1' : 'border-transparent'}"
            >
            {tag} {tags.selectedTag === tag ? '×' : ''}
        </div>
